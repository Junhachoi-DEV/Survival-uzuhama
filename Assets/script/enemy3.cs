...
public class enemy3 : MonoBehaviour
{
    ...
    IEnumerator attacking()
    {
        ...
        audio.Play();
        rigid.AddForce(Vector2.right * (is_check ? 6 : -6), ForceMode2D.Impulse);
        yield return new WaitForSeconds(0.2f);
        gameObject.tag = "big_enemy";
        gameObject.layer = 9;
        animator.SetBool("attack_ready", false);
    ...
    IEnumerator on_damage()
    {
        ...
        else
        {
            ...
            destroy_this();
            GameObject ins_die = obj_m.Make_obj("enemy_33_die");
            ins_die.transform.position = transform.position;
            player.play_sounds("audio_die_en3");
        }
    }
    void destroy_this()
    {
